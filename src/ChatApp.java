...
public class ChatApp {
					...
					while (true) {
						...
						while (keyIterator.hasNext()) {
							...
							if (key1.isAcceptable()) {
								...
								int listeningPort = socket.getPort() - recPort;
	...
	public static void checkUserInput(String userInput) {
		...
		case "send":
			if (flags.length > 2) {
				...
				if (ID.size() == 0) {
					System.out.println("The ID does not exis 1.");
					break;
				}
	...
	public static void connectTcp(String destinationIP, int portNumber) {
		...
		try {
			...
			connectChannel.bind(new InetSocketAddress(getPort() + number));
			number++;
			recPort++;
		...
		try {
			...
			while (keyIterator.hasNext()) {
				...
				String currentIPOnChannel = ((SocketChannel) key5.channel())
						.getRemoteAddress().toString().substring(1).split(":")[0];

				if (key5.isWritable()) {
					if (currentIPOnChannel.equals(ipFromList)) {
						SocketChannel channel = (SocketChannel) key5.channel();
						try {
